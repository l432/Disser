...
Приклади ВАХ, виміряних при однаковій температурі в умовах U4SDB (порожні точки) та без нього (заповнені точки).
$T$, : 190 (кола), 240 (квадрати).
}%
\end{figure}
...
та ВБШ при нульовому зміщенні структур SSDB в умовах U4SDB (2) та без нього (1).
Точки --- експеримент,
...
залежності Річардсона для структур SSDB за умов U4SDB (2, 4) та без нього (1, 3) побудовані в інтервалі
...
$\Theta=[1+(qD_{ss}\delta)/(\varepsilon_0\varepsilon_i)]^{-1}$,

Таблиця~\ref{tabSDBPar}.

\begin{table}
\caption{Параметри, визначені для структур Mo/$n$--Si}
\label{tabSDBPar}
\centering
\begin{tabular}{|l|c|c|}
\hline
...
структур SSDB за умов U4SDB (2) та без нього (1) побудовані в інтервалі
температур $200\div330$~K.
...
gamma--p (\ref{eqGammaP})
Fib0 (\ref{eqFb0T})
співвідношення сігм (\ref{eqGigFSigG})
...
Залежність $n_\mathrm{id,H}kT$ від оберненої температури для структур SSDB за умов U4SDB (2) та без нього (1).
...
%\begin{eqnarray}
%\label{eqIp}
%I_s=AA^*\,T^2\frac{4C_p\,\pi\,\gamma_p\,\eta_b^{2/3}kT}{9V_{bb}^{2/3}q}\times\nonumber\\
...
(a) Температурні залежності висоти бар'єру НТКС структур SSDB за умов U4SDB та без УЗН.
...
(a) Зворотні гілки ВАХ, виміряні за умов УЗН (порожні точки) та без нього (заповнені точки) при різних температурах.
